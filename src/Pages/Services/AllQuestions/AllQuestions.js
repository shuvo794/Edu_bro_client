...
const AllQuestions = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://edu-bro-server.onrender.com/allQuestions?page=${page}&&size=${size}&&department=${department}&&year=${year}&&semester=${semester}&&search=${search}`
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    <div className="container text-black mt-5 mb-5">
      <div className="row align-items-center">
        <div className="col-md-6">
          <div className="mb-5">
            <h1 className="user-desire-question">Find All Questions</h1>
          </div>
        </div>
        <div className="col-md-6">
          ...
          {questions.length === 0 ? (
            ...
            <div className="row">
              {questions?.map((question) => (
                <QuestionCart key={question.id} data={question}></QuestionCart>
              ))}
            </div>
          )}
        </div>
