...
const AdminAssignment = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/getAllAssignments`)
      .then((res) => res.json())
  ...
  const handleUpdate = (id) => {
    fetch(`http://localhost:5000/AssignmentStatusUpdate/${id}`, {
      method: "PUT",
    ...
    <div className="container all-assignment-container">
      <div className="text-center pb-3">
        <h1 className="mb-2 text-center pt-2">
          Total Admin Assignment{" "}
          <span style={{ color: "#007aff" }}>{assignments?.length}</span>{" "}
        </h1>
      </div>
      ...
      <table className="table table-gray" style={{ width: "100%" }}>
        <thead>
          <tr
            ...
            style={{ assignment: "1px solid red" }}
          >
            <th>Number</th>
            <th>Subject</th>
            <th>Uploader</th>
        ...
        className="img-fluid rounded-start w-100 " style={{ height: "50px" }} allow="autoplay"></iframe>
              ...
              <td>
                <div>
                  ...
                  >
                    <option defaultValue={assignment.status}>
                      {assignment.status}
                    </option>
              ...
              <td>
                <button className="btn-style download-btn">
                  <a href={assignment.driveLink} target="_blank">
                    View
                  </a>
                </button>
              </td>
