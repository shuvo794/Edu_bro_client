...
const AllBooks = () => {
  ...
  const allBooksData = useSelector(
    (state) => state.questionsArchiveData.booksData
  );
  ...
  return (
    <div className="container text-black mt-5 mb-5">
      <div className="d-flex my-5 justify-content-center">
        <h1 className="user-desire-question">Books Collection</h1>
      </div>
      {allBooksData.length === 0 ? (
        ...
        <div className="row row-cols-1 row-cols-md-3 g-4">
          {allBooksData?.map((book) => (
            <BooksCart key={book.id} data={book}></BooksCart>
          ))}
        </div>
      )}
    </div>
